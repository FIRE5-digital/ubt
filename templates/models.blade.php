...
<div class="breadcrumbs">
    <div class="ctr"> 
        <ul class="layout inline-list" aria-labelledby="breadcrumbMenu">
            ...
            <li><a href="{{ $model->getUrl() }}">{{ $model->name }}</a></li>
        </ul>
    </div>
</div>
<!-- END BREADCRUMBS -->
...
@section('content')
    ...
    <div class="range ctr">
        <div class="range-intro make-list-intro">
            <div>
                <h1>{{ @$manufacturer->name }} {{ @$model->name }} Leasing</h1>
                {!! @$range->description !!}
            </div>
            <div class="make-logo">
    ...
    <div class="model-table ctr">
            ...
            <div class="search-form table-cell spec">
                <div class="form-group">
                    ...
                    <select class="form-control select-css row-filter" name="spec" id="sortSpec">
                        ...
                        @php($unique = array())
                        @foreach($model->getDerivativesByGroupedModel(price_view()) as $der)
        ...
        @foreach($model->getDerivativesByGroupedModel(\Cookie::get('price_view') ?? 'Personal') as $der)
            @if($der->discontinued&&$der->getDeals()->count()==0) @continue @endif
